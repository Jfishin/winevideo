...
_wv_pkg="$BREW_PREFIX/lib/pkgconfig"
_wv_inc="$BREW_PREFIX/include"
_wv_lib="$BREW_PREFIX/lib"
...
# FFmpeg 7.1 MUST come first so it wins over Homebrew's ffmpeg 8.x (avcodec 62),
# which is symlinked into /usr/local. CX26 bundles FFmpeg 7.1 (avcodec 61) — ABI match.
export PKG_CONFIG_PATH="$FFMPEG_INSTALL/lib/pkgconfig:$_wv_pkg:${PKG_CONFIG_PATH:-}"
export CPATH="$FFMPEG_INSTALL/include:$_wv_inc:${CPATH:-}"
export LIBRARY_PATH="$FFMPEG_INSTALL/lib:$_wv_lib:${LIBRARY_PATH:-}"
